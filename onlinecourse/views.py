...
def show_exam_result(request, course_id, submission_id):
    ...
    # Initialize variables for calculating the total score and storing question results
    total_score = 0
    question_results = []

    # Iterate over the questions in the course
    for question in course.question_set.all():
        # Check if each selected choice is a correct answer or not
        is_correct = set(selected_choice_ids).intersection(question.choice_set.filter(is_correct=True).values_list('id', flat=True))
        question_results.append((question, is_correct))

        # Add the grade of the question to the total score if it is answered correctly
        if is_correct:
            total_score += question.grade

    # Prepare the context for rendering the HTML page
    context = {
        'course': course,
